...
class DictionaryJob {
  ...
  function __construct (Dictionary $dict, string $jobEntry = '') {
    $this->dict = $dict;
    // query matching job entries from the dictionary
  ...
  static public function deconstructJobEntry($jobEntry) {
    ...
    if ($cpos1 !== false) { // arguments present
      if ($cpos2 === false || ($cpos2 <= $cpos1)) { // no ) character found after )
        wire()->warning('Invalid job found: '.$jobEntry);
...
// TODO subtract the time spent on estimation
  ...
  public function execute() {
    ...
    case self::CMD_PROCESS_ALL:
      if (!$this->dict->import($this->options)) {
        wire()->error("Failed processing job '{$this->description}'.");
        return false;
      } else {
        // remove the job
    ...
    case self::CMD_PROCESS_FILE:
      ...
      if (!$this->dict->hasSource()) {
        wire()->warning("[JOB] No files found at '{$this->dict->page->title}' in field '{$this->dict->config->sourcefield}'.");
        break;
      }
      $file = $this->dict->getSource($this->arguments['file']);
      if (is_null($file)) {
        wire()->error('File not found: '.$this->arguments['file']);
        return false;
      }
      ...
      if (!$this->dict->processXMLDictionaryFile($file, $this->options)) {
        wire()->error("Failed processing {$file->name}.");
        return false;
      } else {
        if (!$this->options[DictionarySupport::OPT_DRYRUN])
          $this->dict->removeJob($this->job);
  ...
  public function printDebugInfo() {
    wire()->message("Job Entry: {$this->jobEntry}.", Notice::debug);
  }
