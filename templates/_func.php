...
function renderChildrenOf($pa, $output = '', $level = 0) {
    ...
    foreach ($pa as $child) {
        ...
        if ($child->numChildren && $child->id != 1) {
            if (wire('modules')->isInstalled('DictionarySupport')
	        && $child->template == wire('modules')->DictionarySupport->dictionary_template) {
	       // headwords under a dictionary are rendered by initial letter only
               $output .= '<ul class="dropdown-menu bg-inverse"> &nbsp; ';
               $output .= wire('modules')->DictionarySupport->renderLetterNav($child, null, $class);
               $output .= '</ul>';
            } else if ($child->template != wire('modules')->DictionarySupport->headword_template) {
	       // render the children only if this is not a headword
